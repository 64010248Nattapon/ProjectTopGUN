...
MONGO_DETAILS = "mongodb://tesarally:contestor@mongodb:27018"

client = motor.motor_asyncio.AsyncIOMotorClient(MONGO_DETAILS)

database = client.waterdatatest

water_collection = database.get_collection("waterdatadetail")
...
def water_helper(water) -> dict:
    return {
        ...
        # "Month": water["Month"],
        "Date": water["Date"],
        "station_id": water["station_id"],
        # "w_date": water["w_date"],
        "w_height": water["w_height"],
        "w_discharge": water["w_discharge"],
        # "WaterDrainRate": water["WaterDrainRate"],
    }
...
#retrival water by station_id
async def retrieve_waters_by_station_id(id: int):
    waters = []
    async for water in water_collection.find({"station_id":id}):
        waters.append(water_helper(water))
    return waters
